...
impl StateManager {
    ...
    pub fn register_user(&mut self, sdkey_hash: [u8; 32]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn apply_transition(&mut self, transition: StateTransition) -> Result<(), Box<dyn std::error::Error>> {
        match transition {
            StateTransition::Deposit {
                ...
                if let Some(user_state) = self.user_states.get_mut(&user_sdkey_hash) {
                    ...
                    warn!("Deposit failed: user not found");
                }
            }
            StateTransition::Trade {
                ...
                if let Some(user_a) = self.user_states.get_mut(&user_a_sdkey_hash) {
                ...
                if let Some(user_b) = self.user_states.get_mut(&user_b_sdkey_hash) {
